...
const Pong: React.FC<GameWindowProps> = (props: GameWindowProps) => {
  ...
  const drawPaddleOne = (p5: p5Types) => {
    ...
    if (g.mode.toLowerCase() === "doublepaddle" && g.gameModeConfig != null) {
      p5.rect(
        (g.paddleOneX +
          (g.gameModeConfig as DoublePaddleConfig).paddleXOffset * g.width) *
          scalingRatio,
        (g.paddleOneY +
          (g.gameModeConfig as DoublePaddleConfig).paddleYOffset * g.height) *
          scalingRatio,
        g.paddleWidth * scalingRatio,
        g.paddleHeight * scalingRatio
      );
  ...
  const drawBoundaries = (p5: p5Types) => {
    ...
    if (g.mode.toLowerCase() === "goalkeeper" && g.gameModeConfig != null) {
      ...
      p5.rect(
        ...
        0,
        relativeHeight *
          (1 - (g.gameModeConfig as GoalKeeperConfig).borderSize),
        relativeWidth,
        relativeHeight * (g.gameModeConfig as GoalKeeperConfig).borderSize
      );
      p5.rect(
        ...
        relativeHeight * (g.gameModeConfig as GoalKeeperConfig).borderSize,
        relativeHeight *
          (0.5 - (g.gameModeConfig as GoalKeeperConfig).goalSize / 2)
      );
      p5.rect(
        //bot left
        0,
    ...
    if (getGameStateData().state === 3) {
      ...
      if (getGameStateData().mode.toLowerCase() === "goalkeeper")
        p5.textSize(
          ((relativeWidth * 5) / 100) *
            (1 -
              (getGameStateData().gameModeConfig as GoalKeeperConfig)
                .borderSize)
        );
  ...
  const drawPing = (p5: p5Types) => {
    ...
    p5.text(
      "ping : " + ping + "ms",
      relativeWidth - getGameStateData().paddleWidth * scalingRatio - 100,
      40
    );
    p5.text(
      "state : " + getGameStateData().state,
      relativeWidth - getGameStateData().paddleWidth * scalingRatio - 100,
      55
    );
    p5.pop();
    ...
    p5.text(
      ...
      relativeHeight / 4 +
        (getGameStateData().mode.toLowerCase() === "goalkeeper"
          ? (getGameStateData().gameModeConfig as GoalKeeperConfig).borderSize *
            relativeHeight
          : 0)
    );
    p5.text(
